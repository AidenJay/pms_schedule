...
(
    id                        INTEGER not null,
    job_batch_id              NUMERIC(20) NOT NULL,
    ...
    executor_route_strategy   VARCHAR(150),
    executor_handler          VARCHAR(500),
    executor_param            VARCHAR(1000),
    executor_block_strategy   VARCHAR(150),
    ...
    glue_source                varchar(3000),
    glue_remark               VARCHAR(128),
    glue_updatetime           TIMESTAMP,
    child_jobid               VARCHAR(500),
    trigger_status            INTEGER default 0 not null,
    trigger_last_time         NUMERIC default 0 not null,
    trigger_next_time         NUMERIC default 0 not null
);
-- Add comments to the table
...
(
    ...
    executor_address          VARCHAR(500),
    executor_handler          VARCHAR(500),
    executor_param            VARCHAR(1000),
    executor_sharding_param   VARCHAR(50),
    executor_fail_retry_count INTEGER default 0 not null,
    trigger_time              TIMESTAMP,
    trigger_code              INTEGER not null,
    trigger_msg                varchar(3000),
    handle_time               TIMESTAMP,
    handle_code               INTEGER not null,
    handle_msg                 varchar(3000),
...
(
    id             INTEGER not null,
    registry_group VARCHAR(500) not null,
    registry_key   VARCHAR(500) not null,
    registry_value VARCHAR(500) not null,
    update_time    TIMESTAMP
);
-- Add comments to the table
...
comment on column XXL_JOB_REGISTRY.id
    is '主键ID';
comment
on column XXL_JOB_REGISTRY.registry_group
    is '注册组';
comment
on column XXL_JOB_REGISTRY.registry_key
    is '注册key';
comment
on column XXL_JOB_REGISTRY.registry_value
    is '注册value';
comment
on column XXL_JOB_REGISTRY.update_time
    is '修改时间';
-- Create/Recreate indexes
create
    index I_JOB_REGISTRY_GROUP on XXL_JOB_REGISTRY (REGISTRY_GROUP)
;
create
    index I_JOB_REGISTRY_UPDATE_TIME on XXL_JOB_REGISTRY (UPDATE_TIME)
;
-- Create/Recreate primary, unique and foreign key constraints
...
create
    index I_JOB_LOG_REPORT_DAY on XXL_JOB_LOG_REPORT (TRIGGER_DAY);
...
INSERT INTO xxl_job_info(id, job_group, JOB_BATCH_ID, job_desc, add_time, update_time, author, alarm_email,
...
VALUES (1, 1, 0, '测试任务1', to_date('20181103222131', 'yyyymmddhh24miss'), to_date('20181103222131', 'yyyymmddhh24miss'),
        'XXL', '', 'FIRST', 'demoJobHandler', '', 'SERIAL_EXECUTION', 0, 0, 'BEAN', '', 'GLUE代码初始化',
