...
db需要修改为
$argon2id$v=19$m=4096,t=8,p=1$41uIgY8p2ieXUthJ3y6XhQ$xvHl2GP7jsmeKxmy3BmbJGCNGwzp8L5hdrmZy9d7qAk
...
#### 步骤一：执行器项目中，开发Job类：
    ...
    1、开发一个继承自"IJobHandler"的JobHandler类，实现其中任务方法。
    2、手动通过如下方式注入到执行器容器。
    ```
...
API服务位置：AdminBiz （ com.xxl.job.admin.controller.JobApiController ）
...
请求数据格式如下，放置在 RequestBody 中，JSON格式：
    {
        ...
        "executorBlockStrategy":"COVER_EARLY",      // 任务阻塞策略，可选值参考 ExecutorBlockStrategyEnum
        "executorTimeout":0,                        // 任务超时时间，单位秒，大于零时生效
        "logId":1,                                  // 本次调度日志ID
        "logDateTime":1586629003729,                // 本次调度日志时间
        "glueType":"BEAN",                          // 任务模式，可选值参考 GlueTypeEnum
